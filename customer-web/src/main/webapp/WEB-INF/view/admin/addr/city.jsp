						...
						<div class="widget-main">
							<form id="_form" class="form-inline">
								<label >
									...
									<input name="proCode" type="text" class="form-data input-medium search-data">
								</label> 
								<label >
									<label class="control-label" for="form-field-1"> 市编号： </label>
...
<script type="text/javascript">
	...
	$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
		jQuery(function($) {
			...
			/*//更新市下拉选择框
			function updateCity(proCode){
				$("#_city").empty();
				$.getJSON("${_path}/admin/addr/listCity?proCode="+proCode,function(d) {
	 				$.each(d,function(index,element){
	 					$("#_city").append("<option value='"+element.code+"'>"+element.name+"</option>");
	 				});
				});
				$("#_city").change(function(){
					updateArea($("#_city").val());
				});
			}
			//更新区下拉选择框
			function updateArea(cityCode){
				$("#_area").empty();
				$.getJSON("${_path}/admin/addr/listArea?cityCode="+cityCode,function(d) {
	 				$.each(d,function(index,element){
			...
			//初始化省下拉选择框
			//$("#_pro").empty();
			/* $.getJSON("${_path}/admin/addr/listCity?pageNo=1&pageSize=20",function(d) {
